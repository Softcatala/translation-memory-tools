...
find $project_dir -type f -name '*.po' -print0 | sort -z | while IFS= read -r -d '' file; do
    ...
    posieve --skip-obsolete --coloring-type=html check-rules -s rfile:$pology/upstream/false-friends.rules -s rfile:$pology/upstream/keys.rules -s rfile:$pology/catalan-pology-rules/date-format.rules -s rfile:$pology/catalan-pology-rules/terminology-sc.rules "$file-filtrat.po" > "$file-pology.html"
